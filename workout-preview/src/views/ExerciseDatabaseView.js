...
export function ExerciseDatabaseView({ userId, navigate }) {
    ...
    const [dbSourceTab, setDbSourceTab] = useState('all'); // 'all', 'custom', 'preset'
        ...
        <div className="bg-gray-800 p-6 rounded-lg">
            ...
            <div className="bg-gray-800 p-4 rounded-lg mb-6">
                <div className="grid grid-cols-1 gap-3">
                    {/* DB Source Tabs */}
                    <div className="flex border-b border-gray-700 pb-3 mb-1 gap-2 flex-wrap">
                        <button
                            onClick={() => setDbSourceTab('all')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${dbSourceTab === 'all' ? 'bg-indigo-600 text-white shadow' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
                        >
                            All Exercises ({exercises.length})
                        </button>
                        <button
                            onClick={() => setDbSourceTab('custom')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${dbSourceTab === 'custom' ? 'bg-purple-600 text-white shadow' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
                        >
                            Personalized DB ({exercises.filter(ex => ex.isCustom).length})
                        </button>
                        <button
                            onClick={() => setDbSourceTab('preset')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${dbSourceTab === 'preset' ? 'bg-blue-600 text-white shadow' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
                        >
