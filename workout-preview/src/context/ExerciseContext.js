...
export const ExerciseProvider = ({ children, userId }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const userExercisesData = snapshot.docs.map(doc => ({
                ...doc.data(),
                id: doc.id,
                isCustom: true
            }));

            // Combine user custom exercises with 3,242 System Presets
            const customNames = new Set(userExercisesData.map(e => (e.name || '').trim().toLowerCase()));
            const filteredPresets = (systemPresetExercises || []).filter(
                p => !customNames.has((p.name || '').trim().toLowerCase())
            );

            const combined = [...userExercisesData, ...filteredPresets];
            ...
            const enriched = combined.map(ex => {
                const key = ex.groupKey || (ex.baseName ? ex.baseName.toLowerCase().replace(/[^a-z0-9]/g, '-') : null);
                const baseMaster = (key && familyMasterData[key]) ? familyMasterData[key] : {};
                const mergedMaster = { ...baseMaster, ...(ex.masterData || {}) };
                const isCustom = Boolean(ex.isCustom || ex.createdViaAi || ex.userCreated);
                return {
                    ...ex,
